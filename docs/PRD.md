...
│   │   ├── FileTree.tsx            # 文件树（6大固定分类 + 自定义文件夹 + 拖拽）
...
| 文件夹 | 图标 | 内容 | 项目类型 |
|-------|------|------|---------|
| 画布 | LayoutDashboard | 节点画布文件 | canvas |
| 剧本 | FileText | 剧本文件 | script |
| 图片 | Image | 图片生成项目 | image |
| 视频 | Video | 视频生成项目 | video |
| 音频 | Music | 音频生成项目 | audio |
| 素材库 | FolderOpen | 所有已生成/上传的素材 | - |
...
| 操作 | 触发方式 | 行为 |
|------|---------|------|
| 新建文件 | 右键文件夹 → 新建 → 选择类型 | 创建 CanvasFile 并打开 |
| 重命名 | 右键 → 重命名 / 双击名称 | 内联编辑，回车确认 |
| 删除 | 右键 → 删除 | 删除文件，关闭关联 Tab |
| 移动 | 拖拽到文件夹 / 右键 → 移动到 | 修改 folderId |
| 多选 | Shift+点击 | 范围多选 |

#### 4.2.4 文件 ID 命名规则

文件树项 ID 采用前缀规则：`{category}_{canvasFileId}`
- `canvas_xxx` — 画布文件
- `ai_xxx` — 单文件项目（剧本/图片/视频/音频）的 AI 面板入口
- `script_xxx` — 剧本文件
