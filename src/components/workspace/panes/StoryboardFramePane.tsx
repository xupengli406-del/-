...
interface StoryboardFramePaneProps {
  nodeId: string
}

export default function StoryboardFramePane({ nodeId }: StoryboardFramePaneProps) {
        ...
        <div className="flex-1 space-y-4">
          <div className="w-full aspect-[4/3] rounded-xl overflow-hidden bg-apple-bg-secondary border-2 border-apple-border-light flex items-center justify-center">
            {selectedVersion?.imageUrl ? (
              <img src={selectedVersion.imageUrl} alt={`第${data.index}格`} className="w-full h-full object-contain" />
            ) : (
              <div className="flex flex-col items-center gap-2 text-apple-text-tertiary">
                <ImageLucide size={48} strokeWidth={1} />
                <span className="text-sm">通过 AI 助手面板生成分镜图片</span>
              </div>
            )}
          </div>
        ...
        <div className="w-80 space-y-4">
          ...
          <div>
            ...
            <div className="space-y-1">
              {characters.length === 0 ? (
                <p className="text-xs text-apple-text-tertiary italic">暂无角色</p>
              ) : (
                characters.map((char) => (
                  <button
                    key={char.id}
          ...
          <div>
            ...
            <div className="space-y-1">
              {scenes.length === 0 ? (
                <p className="text-xs text-apple-text-tertiary italic">暂无场景</p>
              ) : (
                <>
                  <button
